...
import { getApiAlbums, apiResetAlbums } from '../actions/api';
...
class Albums extends Component {
  ...
  componentWillUnmount() {
    this.props.dispatch(apiResetAlbums());
  }

  getAlbums = () => {
    this.props.dispatch(getApiAlbums(this.props.navigation.state.params.userId));
  };

  getListData = () => (this.state.searchBarTerms.length > 0 ? this.state.filteredResultData : this.props.apiResultData);

  handleAlbumsPress(albumId) {
    ...
    return (
      <Container paddingBottom={0}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <DefaultListItem onPress={() => this.handleAlbumsPress(item.id)} title={item.title} />
          )}
...
const mapStateToProps = state => ({
  ...state.api,
  apiResultData: utils.orderArrayDescendingById(state.api.apiAlbumsResultData),
});
